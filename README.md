...
[CI](.github/workflows/ci.yml) runs these lanes as one chain of jobs: lint; the source gate and build without network; the kernel re-checks and proof audit on a fresh machine; then the tests, native and Loom lanes. It uploads their evidence. Every push to `main` is checked and never cancelled; only a superseded pull-request run is. For a pull request GitHub runs that workflow, and the scripts it starts, from the pull request's own merge commit, so a green run says that those checks passed rather than that the project's checks passed. [A second workflow](.github/workflows/verify.yml) runs from the default branch, where a pull request cannot edit it, and publishes a check run saying whether every required job of the finished run succeeded and whether the files that decide what the checks do are the default branch's; it reports a change that edits its own checks instead of rejecting it, and it reports nothing until it is itself on the default branch. The checks do not gate merges by themselves: that needs the `lint`, `build`, `proofs`, `test` and `verify` checks to be required on the branch, which only the repository owner can configure. [Backend proof instructions](backend/README.md) describe the separate HOL lane.
