...
```mermaid
flowchart TD
    A[Lean specification and component contracts] --> S[Source language]
    S --> B[Compiler syntax and lowering]
    B --> C[Pancake source]
    C --> D[CakeML compiler]
    D --> E[Native kernel]
...
The diagram describes the intended path. Today the region-digest, byte-copy, decimal-render and reply-table examples traverse syntax, source emission, compilation, and native execution; the reply table is written in the source language. Their model proofs and runtime tests cover different portions of that path; they do not yet form one theorem.

| Layer | Current home | Boundary |
| --- | --- | --- |
| Source language | `lean/DN/Dsl` | Programs as values of a Lean type, their meaning and their compilation, with one proof, in the model of the semantics, for every program the emitter accepts ([decision](decisions/0001-embedding.md)) |
| Component semantics and proofs | `lean/DN/Compiler` | Lean model of Pancake state, memory, locals, FFI oracle, and clock |
| Source emission | `DN.Compiler.Main`, `Checked`, `Syntax`, `Lower`, `Abi` | The checked exported profile — region, echo, render, the reply table and the differential fixtures; unsupported shapes, calls and FFI are refused with a reason |
