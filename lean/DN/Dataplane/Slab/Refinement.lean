    ...
    | cons s rest =>
      ...
      | some s' =>
        have hdead : t.slots s = none :=
          (h.free_dead s (by rw [hf]; exact List.mem_cons_self)).2
        have hss : s' ≠ s := by
          intro he; subst he
...
protected theorem WF.insert {t : Table σ} (h : t.WF) (fd : Nat) (v : σ) :
  ...
  | some s =>
    ...
    · -- dead_free
      intro s' hlt hnone
      have hss : s' ≠ s := by
        intro he; rw [he, upd_self] at hnone; exact absurd hnone (by simp)
  ...
  | none =>
    cases hf : t.free with
    | cons s rest =>
      have hmem : s ∈ t.free := by rw [hf]; exact List.mem_cons_self
      ...
      · -- live_indexed
        ...
        · rw [upd_ne t.slots (some v) hss] at hsome
          ...
          have hfd' : t.slotFd s' ≠ fd := by
            intro he; rw [he, h1] at hidx; exact absurd hidx (by simp)
...
protected theorem WF.remove {t : Table σ} (h : t.WF) (fd : Nat) :
  ...
  | some s =>
    ...
    · -- dead_free
      intro s' hlt hnone
      by_cases hss : s' = s
      · rw [hss]; exact List.mem_cons_self
...
theorem count_insert_replace {t : Table σ} {fd : Nat} {w : σ}
  ...
  | none => rw [lookup, hidx] at h1; exact absurd h1 (by simp)
