...
/* Any progress on a connection renews its idle deadline. The count travels with the
 * renewal so that the two cannot drift apart. */
static void renew(struct connection *c, uint64_t *count) {
    c->active = millis();
    ++*count;
}
...
int main(int argc, char **argv) {
    unsigned port = 0, write_chunk = CAPACITY, idle_ms = 30000, send_buffer = 0;
    ...
    if (nonblocking(listener)) { perror("fcntl"); close(listener); return 1; }
    ...
    if (descriptors.rlim_cur < SLOTS + 4) {
        fputs("too few file descriptors for the slot table\n", stderr);
        close(listener);
        return 1;
    }
    printf("{\"port\":%u,\"slots\":%u,\"buffer_bytes\":%u}\n",
           (unsigned)ntohs(address.sin_port), SLOTS, CAPACITY);
    if (fflush(stdout)) { perror("fflush"); close(listener); return 1; }
    ...
    uint64_t accept_errors = 0, listener_idle_until = 0;
    uint64_t expired_pending = 0, send_renewals = 0, recv_renewals = 0, accept_retries = 0;
    const uint64_t accept_backoff_ms = 50;
    int result = 0;
    while (!stopping) {
        ...
        for (size_t i = 0; i < SLOTS; ++i) {
            ...
            if (c->fd >= 0 && millis() - c->active >= idle_ms) {
                /* Counted separately: a slot still holding a reply must expire too. */
                if (c->length) ++expired_pending;
                close_connection(c); ++expired;
            }
            if (c->fd < 0) free_slot = (int)i;
            events[i+1] = (struct pollfd){c->fd, c->length ? POLLOUT : POLLIN, 0};
        }
        int listening = free_slot >= 0 && millis() >= listener_idle_until;
        events[0] = (struct pollfd){listening ? listener : -1, POLLIN, 0};
        ...
        for (size_t i = 0; i < SLOTS; ++i) {
            ...
            if (c->length) {
                ...
                if (n > 0) {
                    ...
                    c->sent += (size_t)n; renew(c, &send_renewals);
                ...
                if (n > 0) {
                    ...
                    ++calls; bytes += produced; c->length = (size_t)produced;
                    renew(c, &recv_renewals);
                } else if (n == 0 || (errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR)) {
                    close_connection(c);
                }
            }
        ...
        if (events[0].revents & POLLIN) {
            ...
            if (fd >= 0) {
                int shaped = nonblocking(fd);
                /* A small send buffer makes the handover take many rounds, which is how a
                 * slow reader is exercised; zero leaves the kernel's own size alone. */
                if (!shaped && send_buffer) {
                    int size = (int)send_buffer;
                    shaped = setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &size, sizeof size);
                }
                if (shaped) close(fd);
                else { clients[free_slot] = (struct connection){ .fd = fd, .active = millis() }; ++accepted; }
            } else {
                switch (dn_accept_action(errno)) {
                case DN_ACCEPT_RETRY:
                    ++accept_retries;
                    break;
                case DN_ACCEPT_PAUSE:
                    /* Keep serving what is already accepted; come back to the listener
                     * once the resource has had a moment to free up. */
                    ++accept_errors;
                    listener_idle_until = millis() + accept_backoff_ms;
                    break;
                case DN_ACCEPT_FATAL:
                    perror("accept"); result = 1; stopping = 1; break;
                }
            }
        }
    }
    ...
    fprintf(stderr, "{\"kernel_calls\":%" PRIu64 ",\"kernel_bytes\":%" PRIu64
            ...
            ",\"accept_errors\":%" PRIu64 ",\"expired_pending\":%" PRIu64
            ",\"send_renewals\":%" PRIu64 ",\"recv_renewals\":%" PRIu64
            ",\"accept_retries\":%" PRIu64 "}\n",
            calls, bytes, sends, partial, accepted, expired, accept_errors,
            expired_pending, send_renewals, recv_renewals, accept_retries);
    return result;
}
